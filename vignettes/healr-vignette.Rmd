...
```
### Introduction
The process of homoeologous exchange (HE) leads to reciprocal copy number variation between the subgenomes of an allopolyploid (REF). The **healr** package offers a suite of functions to extract meaningful HE related metrics from allopolyploid data using copy number analysis and synteny analysis. Inputs for **healr** are bam files (with whole-genome sequencing reads, either DNA or WGBS) sorted by subgenome (for example using EAGLE-RC (Kuo et al., 2020)), synteny information between the subgenomes obtained using GENESPACE (Lovell et al., 2022) and measures of mappability from genmap (Pockrandt et al., 2020) and GC content, both evaluated within bins in each subgenome. 
...
plot_bins(heal_list = count_list,
          ...
          specific_chr = "chr1",
          colour_map = c("purple3", "goldenrod3"))
```
...
plot_bins(heal_list = cn_list, quick_view_sample = "sample_1", specific_chr = "chr1", plot_cn = TRUE, colour_map = c("purple3", "goldenrod3"))
```
...
In healthy diploids, or established allopolyploids, extensive copy number variation like what is seen in our example is not expected. However, in synthetic allopolyploids, and perhaps also in nascent natural allopolyploids, extensive chromosomal rearrangements have been recorded. One major type of rearragement occurs through homoeologous recombination or exchange (HE) (Mason & Wendel, 2020).

HE leads to copy number variation at each subgenome, but copy number changes reciprocally between the subgenomes. We can therefore start to understand which copy number changes are due to HE by mapping homoeologous regions of the subgenomes to each other and looking for concordance between changes in copy numbers. We define concordance as a sum of copy number equal to the expected ploidy of the organism. So for a tetraploid with diploid progenitors a total copy number of 4 is concordant. Concordance achieved by any combination except 2:2 might be due to homoeologous exchange. 

HE occurs between homoeologous regions, or regions of sufficient similarity that they could pair at meiosis and recombine. Synteny analysis allows us to infer such regions between genomes. The workflow tool GENESPACE finds these regions using conserved ortholog gene order between two genomes. The orthologs used by GENESPACE to infer synteny are called "anchors". These are orthologs involved in a colinear string of gene but also fitting a few additional criteria. Most notably, they are unique representatives when tandem repeats lead to multiple orthologs. In **healr**, the mapping between subgenomes is only done at the level of anchors as bins without anchors cannot mapped between subgenomes directly. We further consider only anchors with orthologs in all subgenomes (in a polyploid with two progenitors this would be all anchors, but with more progenitors it is not necessarily the case). 
Copy number at these anchors is determined based on the overlapping bins. For anchors overlapping with a single bin, or with multiple bins of the same copy number, this is straightforward. For anchors overlapping multiple bins with different copy numbers, we assign to the anchors the copy numbers which lead to the lowest divergence from concordance. If multiple copy number combinations have equal divergence from concordance, the copy number combination with longest overlap length between anchors and bins is taken. The decision to take the combination closest to concordance is ultimately based on an assumption that copy number changes are primarily driven by HE. This might not be the case and could potential inflate the concordance artificially. You can check which method was used to assign a copy number to each anchor set (a group of anchors orthologous to each other between subgenomes). 

You should have a directory containing the syntenicHits directory produced as an output from GENESPACE. 
```{r alignment}
...
We can plot the anchor sets taking each subgenome as reference using plot_alignment(). This takes each subgenome independantly and plots copy number along that genome. Then, the copy number from the orthologous anchor genes of the other subgenome(s) are added at the position of the reference subgenomes anchor.  
```{r plot align, fig.width=7, fig.height=5}
plot_alignment(heal_list = cn_list, alignment = alignment_list, quick_view_sample = "sample_1", specific_chr = c("chr1", "chr2"), colour_map = c("purple3", "goldenrod3"))
```
This clearly reveals that, in this sample, many copy number changes are reciprocal and therefore well explained by homoeologous exchange. 

We can visualize the distribution of copy numbers of all anchor sets as a heat map using plot_heal_heat_map().
...
Most of the pair are concordant. This is strong evidence for an HE driven process of copy number variation in this sample. 
...
Anyways, one can notice from the summary above that the only copy numbers combinations seen above 1% are concordant ones. It seems like there is a bias towards species 1 in the 3 to 1 ratio, but then there is a bias towards species 2 in the 4 to 0 ratio. Whatever this means is unclear, but there is surely some interesting underlying biological knowledge that can be gleamed from these results, especially if we start looking at the specific location of these exchanges. 


### Improving Copy Number Inference

You might have noticed in the alignment plots that some anchor pairs are still discordant, often only very few in a row. This could reflect real copy number changes, maybe due to short scale duplications/deletions, or it could be due to an error in copy number inference. From the strong concordance observed genome wide it seem like copy number at one gene strongly suggests which copy number is likely to be at the other gene. We can therefore leverage this information to improve copy number inference. 

Here, we propose a very unsophisticated approach to try and do this. We start with the assumption that anchor sets which are overlapping unique copy numbers (method="unique") and are concordant (status="concordant") are overlapping bins for which copy number has been correctly inferred. If there is a lot of concordance between the anchors, the vast majority of this concordance is unlikely to be due to chance, thereby justifying our assumption. If you data is not very concordant, then this assumption is no longer well supported. What we then do is to take all the bins assumed to have been correctly assigned to a given copy number and use these to estimate the probability density functions of their respective copy number classes. We only do this for copy number classes capable of being involved in a concordant set, so for allotetraploids with diploid progenitors these are copy numbers 0 to 4. 
We estimate these densities in two dimensions: read counts and GC content, using multivariate kernel density estimation with the function kde2d from the package MASS (Venables & Ripley, 2002). The reason for using these two variables is that read counts is known to be affected by GC content (Benjamini & Speed, 2012). In fact, those familiar with copy number analysis might have wondered why we did not perform any GC content correction prior to copy number inference. The reason for this is that many synthetic polyploid studies produced whole-genome data only for the purpose of methylation analysis and therefore only as WGBS data. In such bisulfite treated data the relationship between GC and read counts depends additionally on the methylation state of the GCs (see [here](https://github.com/chrisamiller/readDepth/issues/16#event-13151257742)). There is no longer a consistent relationship and classic normalization methods based on "healthy" samples or leoss regression do not perform well anymore. If the data is DNA seq the same workflow can be applied but with added GC normalization, this should further improve the outcome. Here, we still try to use GC content but minimize assumptions on the underlying distribution of the trend between GC and read count. However, we still consider that the whole genome of a given sample has the same level of methylation over all so that we can build empirical density distributions using information from the whole genome.

You can infer these empirical densities using get_concordant_density(). Then you can plot the densities using plot_densities(). 
...
```
_**Figure 6:** Contour plots showing the estimated densities with discordant and corrected bins added. Each point is a bin overlapping a discordant anchor set. The colour of the point represents the assigned copy number. Corrected bins have a smaller point overlayed with the colour to which the overlapping anchor was changed to._
...
The outcome is not markedly better, but the information might still be more accurate for the corrected genes. 
...
In the future we will assign a copy number to the non-anchor genes too. This should also reveal many copy number changes in inter anchor regions. Some of these could be invisible in the alignment, if the copy number changes after one anchor and reverts back before the next anchor for instance. Information about HE could therefore be lost in that manner. 
Crucially, the present tool does not explicitly allow inferences of HE. In the future such inference, for example between two related samples or between the starting, non recombined pattern, would be a benefic development. Such inference could take inspiration from nucleotide or codon substitution models, with the fixation of one subgenome being a sink state in selfing organisms. 

Add normalization with progenitors for DNA data. 
